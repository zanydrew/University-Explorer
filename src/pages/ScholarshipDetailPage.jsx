...
/* ── Constants ── */
const ADMISSION_SECTIONS = [
    { title: 'Scholarship Description',  icon: <GraduationCap size={15} />, key: 'description' },
    { title: 'Scholarship Benefits',     icon: <BookOpen size={15} />,      key: 'benefits'    },
    { title: 'Majors/Field of Study',    icon: <BookOpen size={15} />,      key: 'majors'      },
    { title: 'Eligibility Requirements', icon: <BookOpen size={15} />,      key: 'eligibility' },
...
export default function ScholarshipDetailPage({ data, onBack, isWishlisted, onToggleWishlist }) {
    ...
    return (
        <div className="sdp-wrap animate-fade-in">
            ...
            <HeroSection
                ...
                rankBadge={data.ranking ? `#${data.ranking} in Cambodia` : undefined}
                taglineLabel="Accredited by"
                taglineText={data.accreditation}
            />

            {/* ── Quick info row ── */}
            <div className="sdp-inforow">
                {infoItems.map(({ icon, label, value }) => (
                    <div key={label} className="sdp-infocard">
                        <div className="sdp-infocard__icon">{icon}</div>
                        <div>
                            <div className="sdp-infocard__label">{label}</div>
                            <div className="sdp-infocard__value">{value}</div>
                        </div>
                    </div>
                ))}
            </div>
